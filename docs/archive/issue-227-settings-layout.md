...
本次仅调整栏目归属、顺序和必要的自适应布局；已有设置项标题、副标题和选项文案保持不变。新增的栏目标题统一放在九种语言资源中。
...
- Balance refresh while tasks run
  - 运行中刷新间隔
  - 任务结束后继续刷新时间
  - 宽窗口按“运行中 → 结束后”排成单行；窄窗口复用更新操作组的自适应竖排行为
  - 当并排布局会把左侧标题/说明压缩到 220pt 以下时，提前将这两个控件组移到独立一行；控件组本身仍按“运行中 → 结束后”顺序排列，并在更窄时竖排
- Balance information
  - Refresh Now
