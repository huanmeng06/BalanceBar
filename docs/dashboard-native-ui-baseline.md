...
- `DashboardSourceListAppearanceTests`：section row 不再锁 `13pt/.medium` 或 `contentTintColor = .labelColor`；section 标题使用 `controlTextColor`；`tintConfigurationForItem` 按窗口 key / App active 在 `NSTintConfiguration.default` 与 `.monochrome` 之间切换；icon/title 不重复进入 accessibility tree；rebuild / 语言切换 / badge 后 ownership 仍交给 AppKit；不出现自绘 selection、版本特定 RGB 或私有 API
