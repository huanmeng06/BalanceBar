...
- 截断 `contentSurface` 到 safe-area 顶部会制造不同的上下背景组合；该实验已撤销，不得恢复。背景层完全遮挡系统采样的说法尚未证实，不作为后续修复前提。
