...
enum OpenCodexRepositoryError: Error, Equatable {
    ...
    func message(for language: AppLanguage) -> String {
        switch self {
        case .managementUnavailable:
            return tr(.keyOpenCodexRepositoryOpencodexManagementApiIsUnavailable, language: language)
        case .invalidResponse:
            return tr(.keyOpenCodexRepositoryOpencodexReturnedInvalidData, language: language)
        case .switchFailed:
            return tr(.keyOpenCodexRepositoryOpencodexSwitchDidNotComplete, language: language)
        case .verificationFailed:
            return tr(.keyOpenCodexRepositoryOpencodexSwitchVerificationFailed, language: language)
        case .rollbackFailed:
            return tr(.keyOpenCodexRepositoryOpencodexRollbackVerificationFailedTheActualStateMayBePartiallyChanged, language: language)
        }
    }
}
