...
final class AppDelegate: NSObject, NSApplicationDelegate, NSTextFieldDelegate {
    ...
    private lazy var dashboardProviderPages = DashboardProviderPageCoordinator(
        actions: DashboardProviderPageActions(
            onRefresh: { [weak self] in self?.performManualRefresh(source: "dashboard") },
            onSwitchProvider: { [weak self] providerID in self?.switchProvider(providerID) },
            onOpenProvider: { [weak self] providerID in self?.dashboardWindowController.showProvider(providerID) },
        ...
        dashboardWindowController = DashboardWindowController(
            actions: DashboardWindowControllerActions(
                ...
                makeProviderPage: { [weak self] choice in
                    ...
                    return self.dashboardProviderPages.makeDetailPage(
                        choice: choice,
                ...
                prepareForPageReplacement: { [weak self] in
                    ...
                    self.dashboardProviderPages.unmount()
    ...
    private func makeOverviewDashboardPage() -> NSView {
        dashboardProviderPages.unmount()
        return dashboardProviderPages.makeOverviewPage(input: makeDashboardProviderPageInput())
    }
